      ...
      <v-col cols="6">
        <v-container>
          <v-card class="fill-height">
            <v-card-title :style="getLinearGradient(user.topic)"><strong :style="'color : ' + getTopicColor(user.topic)">请输入待优化的代码</strong></v-card-title>
            <v-divider></v-divider>
...
<!--              </v-textarea>-->
      ...
      <v-col cols="6">
        <v-container>
          <v-card class="fill-height">
            <v-card-title :style="getLinearGradient(user.topic)"><strong :style="'color : ' + getTopicColor(user.topic)">人机协同结果</strong></v-card-title>
            <v-divider></v-divider>
            <v-card-text class="d-flex flex-column" style="width: 100%; height: 100%">
...
<!--              {{ result }}-->
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn :color="getTopicColor(user.topic)" text @click="download">下载优化结果</v-btn>
            </v-card-actions>
          </v-card>
        </v-container>
      </v-col>
...
<!--                  <v-text-field label="请输入要优化的代码" v-model="userNameOrEmail" outlined></v-text-field>-->
<!--                </v-col>-->
<!--                <v-col cols="12" class="mb-3">-->
...
export default {
  ...
  data () {
    return {
      code: '',
      result: "暂无优化结果。请在左侧输入代码并点击确认，以获取优化结果。",
      resultBusy: false,
      cmEditor: null,
      cmViewer: null
    }
  ...
  methods: {
    ...
    download() {
      this.downloadStrAsFile(
          '# AutoHub AI 人机协同\n\n## 输入代码\n\n```\n' + this.code + '\n```\n\n## 代码优化结果\n\n' + this.result + '\n\nGenerated by [AutoHub AI](http://AutoHub.eastasia.cloudapp.azure.com/)\n\n' + new Date().toLocaleString(),
          `优化结果-${new Date().toLocaleString()}.md`)
